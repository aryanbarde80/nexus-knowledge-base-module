          ...
          <div class="mt-3 space-y-1">
            ...
            <div v-for="category in rootCategories" :key="category.id">
              ...
              <div class="ml-4 border-l border-slate-200 pl-2 dark:border-slate-800">
                <button v-for="child in store.categories.filter((item) => item.parentId === category.id)" :key="child.id" class="flex w-full items-center justify-between rounded-[10px] px-2 py-1.5 text-left text-xs text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800" @click="store.selectedCategory = child.id">
        ...
        <div class="soft-card p-3.5">
          <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div class="relative flex-1">
              ...
              <input class="input pl-9" placeholder="Search articles, tags, category, author..." @input="store.searchDebounced(($event.target as HTMLInputElement).value)" />
            </div>
          ...
          <div class="mt-3 flex flex-wrap gap-2">
            <button v-for="search in store.recentSearches" :key="search" class="rounded-[10px] bg-slate-100 px-2 py-1 text-xs text-slate-600 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300" @click="store.query = search">{{ search }}</button>
          </div>
        </div>
        ...
        <article v-for="article in store.filteredArticles" v-else :key="article.id" class="card p-4 transition hover:border-blue-200 hover:shadow-enterprise dark:hover:border-nexus-900">
          <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            ...
            <div class="flex shrink-0 items-center gap-1">
              <button class="rounded-[10px] p-2 hover:bg-slate-100 dark:hover:bg-slate-800" @click="store.toggleFavorite(article.id)"><Star class="h-4 w-4" :class="{ 'fill-amber-400 text-amber-400': article.favorite }" /></button>
              <button class="rounded-[10px] p-2 hover:bg-slate-100 dark:hover:bg-slate-800" @click="store.duplicateArticle(article.id)"><Copy class="h-4 w-4" /></button>
              <button class="rounded-[10px] p-2 hover:bg-slate-100 dark:hover:bg-slate-800" @click="store.archiveArticle(article.id)"><Archive class="h-4 w-4" /></button>
              <button class="rounded-[10px] p-2 hover:bg-slate-100 dark:hover:bg-slate-800"><MoreHorizontal class="h-4 w-4" /></button>
            </div>
          </div>
      ...
      <aside class="space-y-4">
        <div class="card p-3.5">
          ...
          <div class="mt-4 space-y-2">
            <button v-for="question in ['How do I create a purchase order?', 'Show onboarding process', 'What is the leave approval workflow?']" :key="question" class="w-full rounded-[12px] bg-slate-50 p-2 text-left text-sm hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700" @click="aiQuestion = question; ask()">{{ question }}</button>
          </div>
          ...
          <div v-if="aiResponse" class="mt-4 rounded-[12px] bg-blue-50 p-3 text-sm dark:bg-blue-950">
            <p>{{ aiResponse.answer }}</p>
            <div class="mt-3 space-y-2">
              <RouterLink v-for="reference in aiResponse.references" :key="reference.id" :to="`/articles/${reference.id}`" class="block rounded-[10px] bg-white p-2 text-xs font-semibold dark:bg-slate-900">{{ reference.title }}</RouterLink>
            </div>
          </div>
        </div>
        ...
        <div class="card p-3.5">
          ...
          <div class="mt-4 space-y-3">
            <div v-for="version in store.versions.slice(0, 4)" :key="version.id" class="rounded-[12px] border border-slate-100 p-3 text-sm dark:border-slate-800">
